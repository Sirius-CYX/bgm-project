...
const MusicFXModule = (function() {
  ...
  /**
   * (Step 1) 创建 5 个原型效果器
   * 全部默认 wet: 0 (关闭)，PanVol 除外
   */
  function _initializeNodes() {
    console.log("MusicFXModule: 正在初始化 5 个原型节点...");
    ...
    // 5. PanVol (声相/音量控制)
    _allEffects.panVol = new Tone.PanVol({ 
      pan: 0, 
      volume: 0, // 0dB = 音量不变
      mute: false
    });
    // 注意：PanVol 没有 wet 属性，它始终是激活的，通过 pan 和 volume 控制
  ...
  return {
    ...
    logCurrentState: function() {
      ...
      for (const name of _effectOrder) {
        ...
        // PanVol 没有 .wet 属性，单独处理
        if (name === "panVol") {
          const pan = effect.pan ? effect.pan.value : 0;
          const volume = effect.volume ? effect.volume.value : 0;
          console.log(`[ACTIVE] ${name.toUpperCase()}: pan=${pan.toFixed(2)}, volume=${volume.toFixed(2)}dB`);
        ...
        if (effect.wet && effect.wet.value > 0) {
          ...
          if (name === "chorus") {
            if (effect.depth) params.push(`depth: ${effect.depth.value.toFixed(2)}`);
            if (effect.frequency) params.push(`frequency: ${effect.frequency.value.toFixed(2)}Hz`);
          }
          if (name === "delay") {
            if (effect.delayTime) {
